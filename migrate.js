...
const createConservationMethodTable = async () => {
  ...
  try {
    ...
    console.log('✓ ConservationMethod table created successfully');
  } catch (err) {
    if (err.message.includes('ORA-00955') || err.message.includes('already exists')) {
      console.log('✓ ConservationMethod table already exists');
    } else {
      throw err;
    }
  } finally {
    ...
    for (const method of methods) {
      try {
        ...
        if (err.message.includes('ORA-00001') || err.message.includes('unique constraint')) {
          ...
          console.error(`  ✗ Error inserting ${method.name}:`, err.message);
        }
      }
    }

    console.log(`✓ Sample conservation methods ready (${inserted} new, ${methods.length - inserted} existing)`);
  } finally {
    await connection.close();
  }
};
